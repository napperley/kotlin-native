...
internal class CodeGeneratorVisitor(val context: Context) : IrElementVisitorVoid {
    ...
    private fun generateVariable(value: IrVariable) {
        ...
        val line = value.line()
        val variableDeclaration = debugInfo.DICreateAutoVariable(context.debugInfo.builder, functionScope, value.descriptor.name.asString(), file, line,  value.descriptor.diType)
        debugInfo.DIInsertDeclarationWithEmptyExpression(context.debugInfo.builder, codegen.vars.load(index) as debugInfo.LLVMValueRef, variableDeclaration, location, LLVMGetInsertBlock(codegen.builder) as debugInfo.LLVMBasicBlockRef)
    }
    ...
    */
    ...
    //-------------------------------------------------------------------------//
    private fun debugLocation(element: IrElement):debugInfo.DILocationRef? {
        ...
        @Suppress("UNCHECKED_CAST")
        return codegen.debugLocation(element.line(), element.column(), diScope as debugInfo.DIScopeOpaqueRef)
    }
   ...
   @Suppress("UNCHECKED_CAST")
    ...
    //-------------------------------------------------------------------------//
    @Suppress("UNCHECKED_CAST")
    private val  VariableDescriptor.diType: debugInfo.DITypeOpaqueRef
    ...
    @Suppress("UNCHECKED_CAST")
    private fun debugInfoBaseType(typeName:String, type:LLVMTypeRef) = debugInfo.DICreateBasicType(context.debugInfo.builder, typeName,
            LLVMSizeOfTypeInBits(codegen.llvmTargetData, type),
            LLVMPreferredAlignmentOfType(codegen.llvmTargetData, type).toLong(), 0) as debugInfo.DITypeOpaqueRef
