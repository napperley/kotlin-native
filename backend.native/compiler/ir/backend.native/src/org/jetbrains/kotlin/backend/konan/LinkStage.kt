...
internal class BeagleboneBlackPlatform(distribution: Distribution)
    : LinuxPlatform(distribution) {

    override val sysRoot = "${distribution.dependenciesDir}/${properties.propertyString("targetSysRoot.linux-beaglebone-black")!!}"
...
internal class LinkStage(val context: Context) {
    ...
    val platform = when (TargetManager.host) {
        KonanTarget.LINUX -> when (targetManager.current) {
            ...
            KonanTarget.BEAGLEBONE_BLACK -> BeagleboneBlackPlatform(distribution)
